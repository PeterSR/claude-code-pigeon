...
func applyConfig(t *testing.T, sid, dir string, logf func(string, ...any)) (string, string, []string) {
	...
	return applyProjectConfig(DefaultNamespace(), sid, dir, cfg, logf)
}
...
func TestRegisterLetsTheSessionOverrideProjectConfig(t *testing.T) {
	...
	if err := register(DefaultNamespace(), "aaaa1111", CurrentRuntime(), currentSessionFacts(), quiet); err != nil {
...
func TestPrivateSessionPublishesNoCwdOrDescription(t *testing.T) {
	...
	if err := register(DefaultNamespace(), "aaaa1111", CurrentRuntime(), currentSessionFacts(), func(string, ...any) {}); err != nil {
		t.Fatalf("register: %v", err)
	}
	e, err := ReadEntry("aaaa1111")
	if err != nil {
